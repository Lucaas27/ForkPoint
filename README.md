...
On startup the app can seed sample data (users, roles, and restaurants) for development. The seed runs only if the data is not present.
...
- The refresh token is rotated on each use, meaning a new refresh token is issued along with the new access token. This limits the window of opportunity if a refresh token is compromised.
